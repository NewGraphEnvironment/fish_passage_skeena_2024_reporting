...
PSCIS crossings `r as.character(my_site)` and `r as.character(my_site2)` are located on `r fpr::fpr_my_pscis_info()`, approximately 22 km southwest of Houston, BC. `r fpr::fpr_my_pscis_info()` flows into Knapper Creek, which joins the Morrice River approximalety 2 km downstream. PSCIS crossing `r as.character(my_site)` is located 1.3 km upstream of the confluence of the subject tributary and Knapper Creek, on spur `r fpr_my_pscis_info(col_pull = road_name, site = my_site)`. Approximately 350 m upstream, crossing `r as.character(my_site2)` is located on `r fpr_my_pscis_info(col_pull = road_name, site = my_site2)`. Both crossings are within the Morrice River watershed group.
...
At the time of assessment, the downstream crossing on road/spur HM7504 was unlikely a barrier to fish passage, and restoration efforts should prioritize the upstream crossing on Gold Creek FSR. However, should restoration activities proceed at crossing `r as.character(my_site)`, replacement with a bridge spanning `r fpr::fpr_my_pscis_info(site = my_site, col_pull = recommended_diameter_or_span_meters)` m is recommended. At the time of reporting in 2025, the estimated cost for the replacement is \$ COMMENTED OUT BELOW.
