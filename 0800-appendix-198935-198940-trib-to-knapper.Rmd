...
PSCIS crossings `r as.character(my_site)` and `r as.character(my_site2)` are located on `r fpr::fpr_my_pscis_info()`, approximately 22km southwest of Houston, BC. `r fpr::fpr_my_pscis_info()` flows into Knapper Creek, which joins the Morice River approximalety 2km downstream. PSCIS crossing `r as.character(my_site)` is located 1.3km upstream of the confluence of the subject tributary and Knapper Creek, on spur `r fpr_my_pscis_info(col_pull = road_name, site = my_site)`. Approximately 350 m upstream, crossing `r as.character(my_site2)` is located on `r fpr_my_pscis_info(col_pull = road_name, site = my_site2)`. Both crossings are within the Morice River watershed group.
