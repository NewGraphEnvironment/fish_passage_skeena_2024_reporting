...
All sites with a design for remediation of fish passage can be found `r if(identical(gitbook_on, FALSE)){knitr::asis_output(paste0("online within the Results and Discussion section of the report found ", ngr::ngr_str_link_url(url_base = params$report_url, anchor_text = "here")))}else{knitr::asis_output("in Table \\@ref(tab:tab-sites-cap) by filtering using Design = yes")}`. In 2024, designs for remediation of fish passage were completed for the following sites:
 ...
 - Station Creek (also known as Mission Creek) – PSCIS crossing 124421 is a Hazelton municipal crossing on 11th Avenue. In 2024, a replacement design was completed by Pacific North Coast Consulting Services, with funding from SERN coordinated through this project. Remediation efforts, led by Skeena Fisheries Commission, Gitksan Watershed Authorities, the Department of Fisheries and Oceans, and the Chicago Creek Community Environmental Enhancement Society, are planned for summer 2025. During the 2024 field visit, an aerial survey was conducted, with the resulting orthomosaics available `r knitr::asis_output(if (gitbook_on) "in Table \\@ref(tab:tab-uav-imagery-cap)" else ngr::ngr_str_link_url(url_base = params$report_url, url_resource = "results-and-discussion.html", anchor_text = "here"))`. Additionally, Waterfall Creek, a tributary to Station Creek, was surveyed from the waterfall downstream to Highway 16. Background information is presented [here](https://www.newgraphenvironment.com/fish_passage_skeena_2021_reporting/).
  
  - Tributary to Owen Creek – PSCIS crossing 197378 is located on Klate Lake Road. In 2024, a replacement design was completed by Pacific North Coast Consulting Services, with funding from SERN coordinated through this project. Klate Lake Road provides access to forestry areas under the tenure of Yinka Dene, as well as Nadina Mountain Provincial Park, which features the [Nadina Mountain Trail](https://www.trailforks.com/trails/nadina-mountain-trail/). Background information is presented [here](https://www.newgraphenvironment.com/fish_passage_skeena_2021_reporting/).
...
Orthoimagery has been gathered for several crossings as part of fish passage restoration planning efforts in the Skeena River watershed. Data has been stored as Cloud Optimized Geotiffs on a cloud service provider (AWS) with select imagery linked to in the collaborative GIS project. Additionally - a tile service has been set up to facilitate viewing and downloading of individual images, provided `r knitr::asis_output(if (gitbook_on) "in Table \\@ref(tab:tab-uav-imagery-cap)" else ngr::ngr_str_link_url(url_base = params$report_url, url_resource = "results-and-discussion.html", anchor_text = "here"))`. 
