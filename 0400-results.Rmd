...
sites_all |>
  ...
  dplyr::select(-idx) |>
    #!!!!!!!!!!!!!!!!!!!!!!!hack to get the designs in asap
  dplyr::mutate(
    design = dplyr::case_when(
      stream_crossing_id %in% c(124421, 197378) ~ "yes",
      T ~ design
    )
  ) |> 
  # make pretty names
...
All sites with a design for remediation of fish passage can be found `r if(identical(gitbook_on, FALSE)){knitr::asis_output(paste0("online within the Results and Discussion section of the report found ", ngr::ngr_str_link_url(url_base = params$report_url, anchor_text = "here")))}else{knitr::asis_output("in Table \\@ref(tab:tab-sites-cap) by filtering using Design = yes")}`. In 2024, designs for remediation of fish passage were completed for the following sites:
 ...
 - Waterfall Creek – PSCIS crossing 124421 is a Hazelton municipal crossing on 11th Avenue. Waterfall Creek flows into Station Creek downstream of this site. Remediation efforts, led by Gitksan Watershed Authorities, in collaboration with SERNbc, Skeena Fisheries Commission, the Department of Fisheries and Oceans, Gitsxan House representives adn the Chicago Creek Community Environmental Enhancement Society, are tentatively planned for summer 2025. Through this year's project - SERNbc funded the  engineering design for a bridge to replace the open bottom culverts - drafted by Pacific North Coast Consulting Services.  In 2024  a field visit was conducted which included a habitat survey from the waterfall located above town to Highway 16.  Additionally  an aerial survey was conducted with a drone to map the historic Waterfall Creek wetland areas located north of Highway 16/10th avenue (east of Churchill Street) to support restoration planning (orthomosaics available `r knitr::asis_output(if (gitbook_on) "in Table \\@ref(tab:tab-uav-imagery-cap)" else ngr::ngr_str_link_url(url_base = params$report_url, url_resource = "results-and-discussion.html", anchor_text = "here"))`). Reporting updated with 2024 data along with background information for the Station Creek and Waterfall Creek watersheds are presented [here](https://www.newgraphenvironment.com/fish_passage_skeena_2021_reporting/) and [here](https://www.newgraphenvironment.com/fish_passage_bulkley_2022_reporting/).
...
BC Ministry of Forests, and Water, Land and Resource Stewardship, Morice Water Monitoring Trust, and Northwest Research and Monitoring [@morgan2024biiwenii]. 
...
All sites where remediation of fish passage has been completed can be found `r if(identical(gitbook_on, FALSE)){knitr::asis_output(paste0("online within the Results and Discussion section of the report found ", ngr::ngr_str_link_url(url_base = params$report_url, anchor_text = "here")))}else{knitr::asis_output("in Table \\@ref(tab:tab-sites-cap) by filtering using Remediation = yes")}`. In 2024, remediation of fish passage was completed at the following site:

  - In 2024,  PSCIS crossing 198217 on Tributary to Skeena River, located on the Sik-E-Dakh Water Tower Road was replaced with a clear-span bridge
...
Following replacement of Taman Creek (PSCIS crossing 197967) on Highway 16, effectiveness monitoring was conducted in 2024.  Background information can be reviewed [here](https://www.newgraphenvironment.com/fish_passage_skeena_2021_reporting/taman-creek---197967---appendix.html) with results of the 2024 monitoring presented in [Taman Creek - 197967 - Appendix](#taman).

<br>

## Aerial Imagery

Orthoimagery and elevation model rasters are presented `r knitr::asis_output(if (gitbook_on) "in Table \\@ref(tab:tab-uav-imagery-cap)" else ngr::ngr_str_link_url(url_base = params$report_url, url_resource = "results-and-discussion.html", anchor_text = paste0("at", params$report_url, "/results-and-discussion.html")))`. 
