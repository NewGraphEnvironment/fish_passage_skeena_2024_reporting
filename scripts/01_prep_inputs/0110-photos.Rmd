...
#this can be run for the ow photos
...
Specific to skeena 2024 - Copy the renamed photos from mergin to the site specific folders on onedrive that contain the extra photos.

```{r move-photos}

#these are the photos we are copying
files_to_copy <- fs::dir_ls(dir_photos_mergin_raw, recurse = T, type = "file") %>% 
  .[!grepl("\\.txt$", .)]   # Exclude files ending in .txt 

# make the paths for where we are copying the photos to
path_to_paste <- dir_photos_onedrive
photos_to_paste <- stringr::str_remove(files_to_copy, "^.*photos/")
files_to_paste <- fs::path(path_to_paste, photos_to_paste)


# Copy files using from mergin to onedrive
...
CANT USUALLY DO THIS UNTILL WE SUBMIT THE DATA AND GET THE PSCIS IDS BUT WE HAVE NO NEW SITES IN THE SKEENA SO WE ALREADY HAVE THE IDS...

```{r dir-site-create-fiss}

# copy the gpkg to the new location - this is just as easily done by hand. Named it weird
form_raw <- fs::path_expand(fs::path("~/Projects/gis/", params$gis_project_name, 'form_fiss_site_newgraph.gpkg'))
form_new <- fs::path_expand(fs::path("~/Projects/gis/", params$gis_project_name, 'data_field/2024/form_fiss_site_newgraph_2024.gpkg'))
...
# create new columns used for naming
...
# create new folders
path_photos_no_metadata <- fs::path("/Users/lucyschick/Library/CloudStorage/OneDrive-Personal/Projects/2024-072-sern-skeena-fish-passage/data/photos/photos_no_metadata")
...
# Copy files using from folder `photos/sorted` to `photos/photos_no_metadata`
purrr::map2(files_to_copy, files_to_paste, ~ fs::file_copy(.x, .y, overwrite = TRUE))
...
photos_dry_run <- fpr::fpr_photo_remove_dupes(paste0(dir_photos_onedrive, "/sorted"))

# # use fpr_photo_remove_dupes to see the triplicated photos
photos_dry_run3 <- fpr::fpr_photo_remove_dupes(paste0(dir_photos_onedrive, "/sorted"),
                                               min_replicates = 3)

# # # actually run the removal of the first un-renamed photo
# fpr::fpr_photo_remove_dupes(paste0(dir_photos_onedrive, "/sorted"),
#                             dry_run = F)
...
photos_dry_run_after <- fpr::fpr_photo_remove_dupes(paste0(dir_photos_onedrive, "/sorted"))
...
# Now there should be no duplicated of any kind, this object should be empty. 
photos_dry_run_after <- fpr::fpr_photo_remove_dupes(dir_photos_onedrive)
...
dirs_to_copy <- fs::path_expand(fs::path(dir_photos_onedrive, phase1_rename$my_crossing_reference))
dirs_to_paste <- fs::path_expand(fs::path(dir_photos_onedrive, phase1_rename$pscis_crossing_id))
  
#Copy over the photos
fs::dir_copy(dirs_to_copy, dirs_to_paste)

# Leave the old folders there in case we need them - not sure if some scripts full photos using the modelled crossing ID...
